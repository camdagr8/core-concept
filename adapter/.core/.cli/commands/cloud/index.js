...
const formatDestination = (val, props) => {
    ...
    val = String(val).replace(/^\/cloud\/|^cloud\/|^cloud/i, `${cwd}/src/app/cloud/`);
...
const overwritable = (prompt) => {
    ...
    try {
        overwrite = prompt.override['overwrite'] || prompt.history('overwrite').value;
    } catch (err) {
        overwrite = true;
    }
...
const collectable = (prompt) => {
    ...
    try {
        collection = prompt.override['collection'] || prompt.history('collection').value;
    } catch (err) {
        collection = false;
    }
...
const HELP = () => {
    ...
    console.log('Example:');
    console.log('');
    console.log('  arcli cloud --name user --collection _User --definitions "userSave userFetch"');
    console.log('');
};
    ...
    return {
        properties: {
            destination: {
                ...
                message: `Enter file path. Example: ${chalk.cyan('cloud/user/index.js')}`,
            },
            overwrite: {
                required: true,
                pattern: /^y|n|Y|N/,
                message: '',
                description: `${chalk.white('Overwrite existing cloud file?')} ${chalk.cyan('(Y/N):')}`,
                ask: () => {
                    try {
                        let destination = prompt.override['destination'] || prompt.history('destination').value;
                        return fs.existsSync(formatDestination(destination, props));
                    } catch (err) {
                        return false;
                    }
                },
            ...
            definitions: {
                ...
                message: `Specify the name[s] of Parse.Cloud.define() definitions. Example: ${chalk.cyan('userSave userFetch contentSave')}`,
            ...
            beforeSave: {
                description: `${chalk.white('Before Save?')} ${chalk.cyan('(Y/N):')}`,
                type: 'string',
                pattern: /^y|n|Y|N/,
                before: beforeBoolean,
            ...
            afterSave: {
                description: `${chalk.white('After Save?')} ${chalk.cyan('(Y/N):')}`,
                type: 'string',
                pattern: /^y|n|Y|N/,
                before: beforeBoolean,
                ask: () => collectable(prompt),
            ...
            beforeDelete: {
                description: `${chalk.white('Before Delete?')} ${chalk.cyan('(Y/N):')}`,
                type: 'string',
                pattern: /^y|n|Y|N/,
                before: beforeBoolean,
                ask: () => collectable(prompt),
            ...
            afterDelete: {
                description: `${chalk.white('After Delete?')} ${chalk.cyan('(Y/N):')}`,
                type: 'string',
                pattern: /^y|n|Y|N/,
                before: beforeBoolean,
                ask: () => collectable(prompt),
            ...
            beforeFind: {
                description: `${chalk.white('Before Find?')} ${chalk.cyan('(Y/N):')}`,
                type: 'string',
                pattern: /^y|n|Y|N/,
                before: beforeBoolean,
                ask: () => collectable(prompt),
...
const ACTION = ({ opt, props }) => {
    ...
    prompt.get(schema, (err, input) => {
        ...
        message(`A new Parse.Cloud file will be created with the following options:`);
...
const COMMAND = ({ program, props }) => program.command(NAME)
    ...
    .option('-d, --destination [destination]', 'Cloud function parent directory.')
    .option('-o, --overwrite [overwrite]', 'Overwrite existing cloud function.')
    .option('-c, --collection [collection]', 'Database collection for before/after hooks. Example: _User')
    .option('--definitions [definitions]', 'Parse.Cloud.define() definitions. Example: "userSave userDelete"')
    .option('--beforeFind [beforeFind]', 'Include beforeFind definition. Only applicable if --collection is specified.')
    .option('--beforeDelete [beforeDelete]', 'Include beforeDelete definition. Only applicable if --collection is specified.')
    .option('--beforeSave [beforeSave]', 'Include beforeSave definition. Only applicable if --collection is specified.')
    .option('--afterDelete [afterDelete]', 'Include afterDelete definition. Only applicable if --collection is specified.')
    .option('--afterSave [afterSave]', 'Include afterSave definition. Only applicable if --collection is specified.')
    .on('--help', HELP);
